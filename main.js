document.addEventListener("DOMContentLoaded", function(event) {
  ...
  let naturalMajor = new Scale(key, "1 2 3 4 5 6 7");
  let harmonicMinor = new Scale(key, "1 2 b3 4 5 b6 7");

  let neck = new Neck(24, 7, gsDrop7);
  // neck.filter(naturalMajor);
  neck.filter(naturalMinor);
  neck.drawTable("neck");
});

class Fret {
  constructor(note, interval, scaleDeg) {
    this.note = note;
  ...
  mix(color1, color2, weight) {
    ...
    var color = "#";
    for (var i = 0; i <= 5; i += 2) {
      var v1 = h2d(color1.substr(i, 2)), v2 = h2d(color2.substr(i, 2)),
  ...
  drawBound(cell) {
    ...
    lowerRight.innerHTML =
        undefined != this.trueInterval ? this.trueInterval : "";
    cell.style.opacity = 0 != this.interval ? 1 : 0.25;
    cell.style.backgroundColor =
        0 != this.interval ? this.mix("2980b9", "e74c3c", this.interval / 7)
                           : "#ffffff";
  ...
  filter(scale) {
    for (let fret = 0; fret < this.getFrets(); fret++) {
      for (let string = 0; string < this.getStrings(); string++) {
        let actualFret = this.grid[fret][string];
        let interval =
            scale.notes.findIndex(element => actualFret.note == element);
        actualFret.interval = -1 != interval ? scale.indices[interval] : 0;
        actualFret.scaleDeg = interval + 1;
        actualFret.trueInterval = scale.intervals[interval];
      }
    }
  }
...
class Tune {
  ...
  setString(string, note) { this.strings[string - 1] = note; }
...
class Chromatic {
  ...
  setRoot(note) { this.root = this.scale.findIndex(root => root == note); }

  getNote(ix) { return this.scale[(this.root + ix) % 12]; }
}
